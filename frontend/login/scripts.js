...
const CONFIG = {
  API_URL: "https://jpass.onrender.com", // URL correta do backend
...
class GerenciadorLogin {
  ...
  validarSenha() {
    ...
    if (senha.length < CONFIG.SENHA_MIN_CARACTERES) {
      this.mostrarErro(
        elementoErro,
        `A senha deve ter no mínimo ${CONFIG.SENHA_MIN_CARACTERES} caracteres`
      )
      return false
    }
  ...
  tratarErroLogin(erro) {
    ...
    const mensagem =
      mensagensErro[erro.code] || "Erro ao realizar login. Tente novamente"
